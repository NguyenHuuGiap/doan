  ...
  task remake_data: :environment do
    ...
    User.all.each do |user|
      user_course = UserCourse.create!([
        user_id: user, active: true, course_id: course
      ])
      UserSubject.create!([
        status: 0, user_id: user, course_id: course, user_course_id: user_course,
        course_subject_id: CourseSubject.first, start_date: Time.zone.now,
        end_date: Time.zone.now + 2.days, during_time: 2, progress: 1
      ])
    end
    ...
    Test.all.each do |test|
      Question.create!([
        {content: "abc", level: 1, subject_id: test.user_subject.first.subject_id},
        {content: "abc1", level: 1, subject_id: test.user_subject.first.subject_id},
        {content: "abc2", level: 1, subject_id: test.user_subject.first.subject_id},
        {content: "abc3", level: 1, subject_id: test.user_subject.first.subject_id}
      ])
    end
    ...
    Question.all.each do |question|
      Answer.create!([
        {content: "aaaa", is_correct: true, question_id: question},
        {content: "aaaa1", is_correct: false, question_id: question},
        {content: "aaaa3", is_correct: false, question_id: question},
        {content: "aaaa2", is_correct: false, question_id: question},
      ])
    end
  end
end
